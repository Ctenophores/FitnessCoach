...
def overlay_perfect_skeleton_on_video(perfect_seq, video_path, output_path, scale):
    ...
    while True:
        ...
        #draw_skeleton_on_frame(frame, skeleton, width, height, scale)
        draw_skeleton_on_frame(frame, skeleton, width, height, scale)
...
def draw_skeleton_on_frame_ori(frame, skeleton_33x4, width, height, scale):
    # 先把 33个点转像素坐标
    points_2d = []
    for i in range(33):
        x = skeleton_33x4[i, 0]
        ...
        y_px = int((1-y) * height * scale + offset_y)
...
def draw_skeleton_on_frame(frame, skeleton_33x4, width, height,
                           scale, point_radius=4, line_thickness=2):
    xy = skeleton_33x4[:, :2]
    ...
    for i in range(33):
        ...
        x_px = int((x - center_xy[0]) * scale * width + width // 2)
        y_px = int(-(y - center_xy[1]) * scale * height + height // 2)
...
def main():
    ...
    original_video = "PerfectAction/BodyWeightSquats/perfect3.mp4"
    output_video = "overlaid_skeleton.mp4"

    overlay_perfect_skeleton_on_video(perfect_seq,
